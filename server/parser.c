...
int parserParseConfigFile (char *file)
{
  ...
  if (import_dir) {
    ...
    logMsg (DBG_SETUP, "\tAdding directory [%s] to the import stack...\n",
            import_dir);
#endif
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\tError adding directory [%s] to import stack.\n",
              import_dir);
#endif
  ...
  if (config_file == NULL) {
    ...
    logMsg (DBG_GEN, "Couldn't find peep configuration file at: %s\n", file);
...
#if DEBUG_LEVEL & DBG_SETUP
  logMsg (DBG_SETUP, "\tStarting parse of: [%s].\n", file);
#endif
  ...
  while (fgets (buffer, sizeof (buffer), config_file)) {
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\tParsing 'general' section...\n");
#endif

      if (parserParseGeneral (config_file) < 0) {

        logMsg (DBG_GEN, "Error parsing general information...\n");
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\tParsing 'class' section for [%s]...\n", classname);
#endif

      if (parserParseClass (classname, config_file) < 0) {

        logMsg (DBG_GEN, "Error parsing class: %s...\n", classname);
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\tSkipping section for client: [%s]...\n", tok.token);
#endif
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\tLoading event:\n");
#endif

      if (parserParseEvent (config_file) < 0) {

        logMsg (DBG_GEN, "Error parsing an event...\n");
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\tLoading state:\n");
#endif

      if (parserParseState (config_file) < 0) {

        logMsg (DBG_GEN, "Error parsing a state...\n");
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\tImporting configuration file: [%s]\n", tok.token);
#endif

      if (parserParseConfigFile (tok.token) < 0) {

        logMsg (DBG_GEN, "import failed! Aborting!\n");
    ...
    if (!strcasecmp (tok.token, PARSER_THEME_TOKEN)) {
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\n");
      logMsg (DBG_SETUP, "Loading theme file: [%s]...\n", tok.token);
#endif

      if ((infile = fopen (tok.token, "r")) == NULL) {

        logMsg (DBG_GEN, "Error opening theme file %s: %s\n", tok.token,
                strerror (errno));
      ...
      xml_string = malloc (parserGetFileSize (tok.token) + 1);
      ...
      if (!xmlParseTheme (xml_string, sound_path, &event_cnt, &state_cnt)) {
        ...
        logMsg (DBG_GEN, "Error parsing theme file: %s\n", tok.token);
        success = PARSER_THEME_ERROR;
        goto parser_failure;
...
FILE *parserImportConfig (char *file)
{
  ...
  for (dir_ptr = import_stack; *dir_ptr; dir_ptr++) {
    ...
    char *import_path = malloc (strlen (file) + strlen (*dir_ptr) + sizeof
                                PARSER_PATH_SEP + 1);
...
int parserIsMyClass (char *host)
{
  ...
  if (gethostname (localhost, 1024) < 0) {
    ...
    logMsg (DBG_GEN, "Error retrieving hostname: %s\n", strerror (errno));
    return 0;
  ...
  if ((entry = gethostbyname (localhost)) == NULL) {
    ...
    logMsg (DBG_GEN, "Error getting host by name for %s: %s\n", localhost,
            strerror (errno));
    return 0;
...
int parserParseGeneral (FILE *config_file)
{
  ...
  while (fgets (buffer, sizeof (buffer), config_file)) {
...
#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\t\tChecked configuration file of appropriate version.\n");
#endif
    ...
    if (!strcasecmp (tok.token, PARSER_SND_PATH_TOKEN)) {
      ...
      parserTokenize (&tok);
      sound_path = calloc (strlen (tok.token) + 2, sizeof *sound_path);
      strcpy (sound_path, tok.token);

#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\t\tSet top-level sound path to: %s.\n", sound_path);
#endif
...
#if DEBUG_LEVEL & DBG_SETUP
        logMsg (DBG_SETUP, "\t\tAdded broadcast port: [%d].\n", atoi (tok.token));
#endif
...
#if DEBUG_LEVEL & DBG_SETUP
                logMsg (DBG_SETUP, "\t\tAdded class [%s] to identifier string.\n", class);
#endif
                break;
...
int parserParseEvent (FILE *config_file)
{
  ...
  while (fgets (buffer, sizeof (buffer), config_file)) {
    ...
    if (!strcasecmp (tok.token, PARSER_EVENT_PATH_TOKEN)) {
      ...
      parserTokenize (&tok);
      snd_path = malloc (strlen (tok.token) + strlen (sound_path) + 1);
      strcpy (snd_path, sound_path);
      strcat (snd_path, tok.token);

#if DEBUG_LEVEL & DBG_SETUP
      logMsg (DBG_SETUP, "\t\tAdding sounds from: [%s]...\n", snd_path);
#endif
    ...
    if (!strcasecmp (tok.token, PARSER_END_TOKEN)) {
      ...
      if (!strcasecmp (tok.token, PARSER_EVENT_TOKEN)) {
        ...
        if (name == NULL || snd_path == NULL) {
          ...
          logMsg (DBG_GEN, "Error loading event:\n");
          logMsg (DBG_GEN, "\tFailed to provide a name identifier or a valid path\n");
#endif
...
int parserLoadEventSndDir (char *name, char *snd_path)
{
  ...
  if ((snd_cnt = parserScanDir (snd_path, &namelist, parserScanCompar)) < 0) {
    ...
    logMsg (DBG_GEN, "Error scanning directory: %s\n", strerror (errno));
    return 0;
    ...
    while (snd_cnt--) {
      ...
      path = malloc (strlen (snd_path) + strlen (namelist[snd_cnt]->d_name) + 2);
...
int parserParseThreshold (FILE *config_file, int thresh_cnt)
{
  ...
  while (fgets (buffer, sizeof (buffer), config_file)) {
    ...
    if (!strcasecmp (tok.token, PARSER_PATH_TOKEN)) {
      ...
      path = malloc (strlen (tok.token) + strlen (sound_path) + 2);
    ...
    if (!strcasecmp (tok.token, PARSER_END_TOKEN)) {
      ...
      if (!strcasecmp (tok.token, PARSER_THRESHOLD_TOKEN)) {
        ...
        if (path != NULL && h_bound != 0.0) {
          ...
          logMsg (DBG_GEN, "Error attempted to add incorrect threshold entry.\n");
          logMsg (DBG_GEN, "Bailing...\n");
          shutDown ();
                           ...
                           char *snd_path, double fade)
{
  ...
  if (state_cnt >= mixerSBuffs () ) {
    ...
    logMsg (DBG_DEF, "Attempted to load one too many state sounds. Skipped: %d\n",
...
int parserImportStackAdd (char *dir)
{
  ...
  if (!import_stack) {
    ...
    import_stack = malloc (sizeof *import_stack);
    import_stack[0] = NULL;
  ...
  dir_copy = malloc ((strlen (dir) + 1) * sizeof (char));
  strcpy (dir_copy, dir);
                   ...
                   int (*compar)(const void *, const void *))
{
  ...
  while ((dir_ptr = readdir (dfile)) != NULL) {
     ...
     */
    items++;
    list_ptr = (struct dirent **)realloc (list_ptr,
                                          items * sizeof (struct dirent *));
...
int parserGetFileSize (char *path)
{
  ...
  if (stat (path, &file_stat) < 0) {
    ...
    logMsg (DBG_GEN, "Error stat-ing file to obtain file size: %s\n",
            strerror (errno));
    return -1;
...
short *parserLoadSoundFile (size_t *array_size, char *path)
{
  ...
  if ((infile = fopen (path, "r")) == NULL) {
    ...
    logMsg (DBG_DEF, "Error opening file at %s: %s\n", path, strerror (errno));
    logMsg (DBG_DEF, "All attempts to play that file will be ignored...\n");
    return NULL;
  ...
  *array_size = parserGetFileSize (path) / 2;
  ...
  if ((snd = calloc (*array_size, sizeof *snd)) == NULL) {

    logMsg (DBG_DEF, "Error allocating memory: %s\n", strerror (errno));
    return NULL;
  ...
  logMsg (DBG_DEF, "\t\tLoaded [%s]: %d bytes.\n", path, (*array_size * 2));
