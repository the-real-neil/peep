...
int serverAddBroadcastPort (int port)
{
	...
	if (broadcast_list == NULL) {
		...
		if ((broadcast_list = (BROADCAST *)malloc (sizeof (BROADCAST))) == NULL)
		...
		if ((cur_bc->next = (BROADCAST *)malloc (sizeof (BROADCAST))) == NULL)
			return SERVER_FAILURE;

		memset (cur_bc->next, 0, sizeof (BROADCAST));
		cur_bc = cur_bc->next;
...
int initializeBroadcast (void)
{
	...
	log (DBG_SRVR, "Opening udp socket for initial broadcast...\n");
#endif

	if ((prot = getprotobyname ("udp")) == NULL) {

		log (DBG_GEN, "Uh Oh! Error resolving protocl info for 'udp': %s\n", strerror (errno));
		return SERVER_FAILURE;
	...
	if ((broadcast_fd = socket (AF_INET, SOCK_DGRAM, prot_no)) < 0) {
		...
		log (DBG_GEN, "Uh Oh! Couldn't open udp socket for broadcasting: %s\n", strerror (errno));
		return SERVER_FAILURE;
	...
	if (bind (broadcast_fd, (struct sockaddr *)&baddr, sizeof (struct sockaddr_in)) < 0) {

		log (DBG_GEN, "Uh Oh! Couldn't bind to udp broadcast port: %s\n", strerror (errno));
		return SERVER_FAILURE;
	...
	if (identifier == NULL) {
		...
		identifier = (MSG_STRING)malloc (2 * sizeof (char));
	...
	for (cur_bc = broadcast_list; cur_bc; cur_bc = cur_bc->next) {
		...
		if (sendto (broadcast_fd, buffer, buf_len, 0,
			...
			log (DBG_GEN, "Warning: received a broadcast error message: %s\n", strerror (errno));
		...
		log (DBG_DEF, "Broadcasting existence to port [%d].\n", cur_bc->port);
...
void receiveUDPPacket (int fd)
{
...
#if DEBUG_LEVEL & DBG_SRVR
		log (DBG_SRVR, "Hmm, it seems a UDP packet did not reach its destination.\n");
		return;
#endif
...
#if DEBUG_LEVEL & DBG_SRVR
		log (DBG_SRVR, "Received UDP packet with bad magic number. Discarding...\n");
#endif
	...
	case PROT_BC_CLIENT:
		...
		msg.body = (void *)malloc (msg.header.len + 1);
		memcpy (msg.body, buffer + sizeof (HEADER), msg.header.len);
		((char *)msg.body)[msg.header.len] = '\0';

#if DEBUG_LEVEL & DBG_SRVR
		log (DBG_SRVR, "Received client broadcast packet.\n");
#endif
...
#if DEBUG_LEVEL & DBG_SRVR
		log (DBG_SRVR, "Received unsupported packet type from broadcast. Discarding...\n");
#endif
		break;
...
void serverProcessClientEventPacket (PACKET *packet, void *data_buffer)
{
	...
	case PROT_CONTENT_EVENT:
		...
		packet->body = (void *)malloc (sizeof (EVENT_BODY));
		 ...
		 */
		event->sound = (char *)malloc ((event->sound_len + 1) * sizeof (char));
		memcpy (event->sound, data_buffer + sizeof (EVENT_BODY) - sizeof (char *), event->sound_len);
...
#if DEBUG_LEVEL & DBG_SRVR
		log (DBG_SRVR, "Got client event with XML content.\n");
#endif

		notice_string = (char *)malloc (msg_len + 1);
...
#if DEBUG_LEVEL & DBG_SRVR
			log (DBG_SRVR, "Error converting XML notice to engine event. Discarding...\n");
#endif
...
#if DEBUG_LEVEL & DBG_SRVR
		log (DBG_SRVR, "Got client event with ENGINE EVENT type content.\n");
#endif
...
void serverConvertNoticeToEngineEvent (EVENT *event, NOTICE *notice)
{
	...
	event->sound     = (char *)malloc (event->sound_len + 1);
...
int serverAddIDClass (char *class)
{
	...
	if (!identifier) {
		...
		if ((identifier = (char *)malloc (strlen (class) + strlen (PROT_CLASSDELIM) + 1)) == NULL)
		...
		if ((identifier = realloc (identifier, strlen (identifier) + strlen (class) + strlen (PROT_CLASSDELIM) + 1)) == NULL)
			return SERVER_ALLOC_FAILED;
...
int serverContainsIDClass (char *class)
{
	...
	char *myclass = (char *)malloc (strlen (class) + strlen (PROT_CLASSDELIM) + 1);
...
#if DEBUG_LEVEL & DBG_SRVR
	log (DBG_SRVR, "Sent response to client.\n");
#endif
