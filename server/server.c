...
int initializeBroadcast (void)
{
    ...
    if (identifier == NULL) {
        ...
        logMsg (DBG_DEF, "Check your classes in peep.conf. Using blank identifier instead.\n");
...
#if DEBUG_LEVEL & DBG_SRVR
        logMsg (DBG_SRVR, "Hmm, it seems a UDP packet did not reach its destination.\n");
        return;
#endif
...
#if DEBUG_LEVEL & DBG_SRVR
        logMsg (DBG_SRVR, "Received unsupported packet type from broadcast. Discarding...\n");
#endif
        break;
...
#if DEBUG_LEVEL & DBG_SRVR
            logMsg (DBG_SRVR, "Error converting XML notice to engine event. Discarding...\n");
#endif
...
int serverAddIDClass (char *class)
{
    ...
    if (!identifier) {
        ...
        if ((identifier = malloc (strlen (class) + strlen (PROT_CLASSDELIM) + 1)) == NULL) {
