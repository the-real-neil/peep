...
void xmlParseClientEvent (char *xml_string, int len, NOTICE *notice)
{
	...
	parse_info = (CLIENT_PARSE_INFO *)calloc (sizeof (CLIENT_PARSE_INFO), 1);
	...
	if (! XML_Parse (parser, xml_string, len, 1) ) {
		...
		log (DBG_GEN, "Uh Oh! There was an XML parsing error: %s\n",
			 (char *)XML_ErrorString ( XML_GetErrorCode (parser) ));
		/* Try to continue */
						  ...
						  const XML_Char **attribs)
{
	...
	client_info->current_tag = (char *)realloc (client_info->current_tag, strlen (tag_name) + 1);
...
void xmlParseClientChar (void *data, const XML_Char *s, int len)
{
	...
	if (!strcasecmp (client_info->current_tag, NOTICE_HOST_TAG)) {
		...
		log (DBG_HOOK, "WARNING: Encountered unsupported XML tag: [%s]\n", client_info->current_tag);
		log (DBG_HOOK, "WARNING: XML text is: [%s]\n", string);
