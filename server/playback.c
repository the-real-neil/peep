...
#if DEBUG_LEVEL & DBG_PLBK
	log (DBG_PLBK, "The playback header is:\n");
	log (DBG_PLBK, "\tmajor ver:      [%d]\n", header.major_ver);
	log (DBG_PLBK, "\tminor ver:      [%d]\n", header.minor_ver);
	log (DBG_PLBK, "\tmax events:     [%d]\n", header.max_events);
	log (DBG_PLBK, "\tevents written: [%d]\n", header.written);
	log (DBG_PLBK, "\tstart pos:      [%d]\n", header.start_pos);
	log (DBG_PLBK, "\tstart time:     [%lf]\n", TP_IN_FP_SECS (header.start_t));
	log (DBG_PLBK, "\tend time:       [%lf]\n", TP_IN_FP_SECS (header.end_t));
#endif
...
void playbackGo (char *start_t, char *end_t)
{
	...
	do {
		...
		plbk_offset = TP_IN_FP_SECS (rec.record.mix_time) - TP_IN_FP_SECS (event_offset);
		cur_time_offset = TP_IN_FP_SECS (current) - TP_IN_FP_SECS (time_offset);

#if DEBUG_LEVEL & DBG_PLBK
		log (DBG_PLBK, "Playback Event:\n");
		log (DBG_PLBK, "\tmix in time:  [%lf]\n", TP_IN_FP_SECS (rec.record.mix_time));
		log (DBG_PLBK, "\tcurrent time: [%lf]\n", TP_IN_FP_SECS (current));
		log (DBG_PLBK, "\tmix offset:   [%lf]\n", plbk_offset);
		log (DBG_PLBK, "\ttime offset:  [%lf]\n", cur_time_offset);
		log (DBG_PLBK, "\tsleeping:     [%lf]\n", plbk_offset - cur_time_offset);
#endif
...
int playbackFileShutdown (void)
{
	...
	case RECORD_MODE:
		...
		log (DBG_PLBK, "On shutdown, the playback header is:\n");
		log (DBG_PLBK, "\tmajor ver:      [%d]\n", header.major_ver);
		log (DBG_PLBK, "\tminor ver:      [%d]\n", header.minor_ver);
		log (DBG_PLBK, "\tmax events:     [%d]\n", header.max_events);
		log (DBG_PLBK, "\tevents written: [%d]\n", header.written);
		...
		if (fseek (stream, 0, SEEK_SET) < 0) {
			...
			log (DBG_GEN, "Error seeking in file: %s", strerror (errno));
			return 0;
