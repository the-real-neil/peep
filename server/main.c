...
int main (int argc, char *argv[])
{
    ...
    if (args_info.playback_mode_given || args_info.record_mode_given) {
        ...
        if (!playbackFileInit (args_info.record_file_arg)) {
            ...
            logMsg (DBG_DEF, "Uh Oh! Couldn't successfully initiliaze playback file! Giving up.\n");
            shutDown ();
...
#if DEBUG_LEVEL & DBG_SRVR
            logMsg (DBG_SRVR, "Received invalid event type or type does not match sound table.\n");
#endif
