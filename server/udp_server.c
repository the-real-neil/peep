...
int serverRealInit (void)
{
	...
	log (DBG_SRVR, "Initializing UDP server routines...\n");
#endif
...
void serverRealStart (void)
{
	...
	data_buffer = (char *)malloc (MAX_UDP_PACKET_SIZE * sizeof (char));
...
#if DEBUG_LEVEL & DBG_SRVR
			log (DBG_SRVR, "Server got connection refused. UDP packet did not reach its destination.\n");
			log (DBG_SRVR, "\n");
#endif
			continue;
...
#if DEBUG_LEVEL & DBG_SRVR
		log (DBG_SRVR, "Server received a packet from: [%s], size: [%d]\n",
...
#if DEBUG_LEVEL & DBG_SRVR
			log (DBG_SRVR, "Received packet with bad magic number. Discarding...\n");
#endif
...
#if DEBUG_LEVEL & DBG_SRVR
			log (DBG_SRVR, "Received still alive from other server. Discarding...\n");
#endif
			break;
...
#if DEBUG_LEVEL & DBG_SRVR
			log (DBG_SRVR, "Apparently a client is still alive...\n");
#endif
...
int serverAddClient (struct in_addr host, unsigned int port)
{
	...
	if ((lease = serverFindLeaseEntry (&host, port)) == NULL)
		lease = serverAddLeaseEntry (&host, port);
			...
			log (DBG_AUTO, "AUTODISCOVERY: Attempted to add client at %s:%d but client already in hostlist.\n",
				 inet_ntoa (host), port);
			log (DBG_AUTO, "AUTODISCOVERY: Updating client expiration time...\n");
#endif
...
#if DEBUG_LEVEL & DBG_AUTO
	log (DBG_AUTO, "AUTODISCOVERY: Sent a response to the client\n");

	{
		...
		for (r = hlhead; r; r = r->nextent)
			i++;
		log (DBG_AUTO, "AUTODISCOVERY: There are [%d] in the hostlist.\n", i);
	}
#endif
...
int serverNotifyClientLease (struct in_addr host, unsigned int port)
{
	...
	if ((buffer = (char *)malloc (sizeof (HEADER) + header.len)) == NULL) {

		log (DBG_GEN, "Error allocating server broadcast buffer: %s\n", strerror (errno));
		return SERVER_FAILURE;
...
#if DEBUG_LEVEL & DBG_AUTO
	log (DBG_AUTO, "AUTODISCOVERY: Adding client [%s]:[%d] to host list...\n",
	     inet_ntoa (*host), port);
#endif

	if (hlhead == NULL) {

		hlhead = (struct leaselist *)calloc (sizeof (struct leaselist), 1);
		p = hlhead;
		...
		p->nextent = (struct leaselist *)calloc (sizeof (struct leaselist), 1);
...
void serverPurgeHostList (void)
{
...

	{
		...
		log (DBG_AUTO, "AUTODISCOVERY: About to purge hostlist with [%d] entries.\n", i);
	}
#endif
...
#if DEBUG_LEVEL & DBG_AUTO
	log (DBG_AUTO, "AUTODISCOVERY: Purged [%d] hosts from hostlist.\n", cnt);
#endif
...
void serverSendStillAlive (void)
{
	...
	if ((buffer = (char *)malloc (sizeof (HEADER) + sizeof (LEASE_BODY))) == NULL) {

		log (DBG_GEN, "Error allocating buffer for still alive packet: %s\n", strerror (errno));
		return;
...
#if DEBUG_LEVEL & DBG_AUTO
		log (DBG_AUTO, "AUTODISCOVERY: sending alive to: %s:%d\n",
...
#if DEBUG_LEVEL & DBG_AUTO
	log (DBG_AUTO, "AUTODISCOVERY: Letting valid clients know we're still alive.\n");
#endif
