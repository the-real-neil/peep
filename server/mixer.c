				...
				unsigned int sbuf)
{
	...
	output = (short *)calloc (sizeof(short), chunk_size);

	/* Initialize the event and state datastructures */
	no_ebuffs = ebuf;
	no_sbuffs = sbuf;

	ebuffs = (EVENT_BUF *)calloc (sizeof(EVENT_BUF), no_ebuffs);
	sbuffs = (STATE_BUF *)calloc (sizeof(STATE_BUF), no_sbuffs);

	/* Allocate the dynamic volume datastructures */
	dyn_mul = (double *)calloc (sizeof(double), no_ebuffs);
				   ...
				   double loc, int flags, unsigned int voice)
{
	...
	log (DBG_MXR, "Sound added to channel [%d]:\n", voice);
	log (DBG_MXR, "\tlength:      [%d]\n", len);
	log (DBG_MXR, "\tstereo pos:  [%lf]\n", loc);
	log (DBG_MXR, "\tfilter flag: [0x%03x]\n", flags);
#endif
...
#if DEBUG_LEVEL & DBG_MXR
	log (DBG_MXR, "\n");
	log (DBG_MXR, "For channel [%d], using dynamic volume multiplier of [%lf].\n",
...

	{
		...
		log (DBG_MXR, "Positions of currently playing sounds:\n");
		...
		log (DBG_MXR, "Total dynamic volume for [%d] channels is: %lf\n", chans,
			 mixerDynTotVol());
...
int mixerAllocNewState (unsigned int state, int thresh_cnt)
{
	...
	sbuffs[state].thresh = (THRESHOLD *)calloc (sizeof (THRESHOLD), thresh_cnt);
							...
							unsigned int snd_cnt)
{
	...
	ptr->state_snd.snd_buf = (short **)calloc (sizeof (short *), snd_cnt);
	ptr->state_snd.len = (unsigned int *)calloc (sizeof (unsigned int), snd_cnt);
				   ...
				   unsigned int no_snd, short *sound, unsigned int len)
{
	...
	if (thresh_index > sbuffs[state].thresh_cnt)
		...
		return MIXER_ALLOC_FAILED;
	else if (sbuffs[state].thresh[thresh_index].state_snd.snd_buf[no_snd] != NULL)
		return MIXER_ALREADY_ALLOC;
...
THRESHOLD *mixerGetThresholdEntry (int j, int index)
{
	...
	if (sbuffs == NULL || sbuffs[j].thresh == NULL || index >= sbuffs[j].thresh_cnt)
		return NULL;
	else
...
void mixer (void)
{
	...
	for (i = 0; i < chunk_size; i += STEREO) {
		...
		for (j = 0; j < no_sbuffs; j++) {
			...
			if (sbuffs[j].vol == 0.0 || state_snd == NULL || state_snd->snd_buf[state_snd->snd_no] == NULL)
				continue;
			else {
				...
				ASSERT (state_snd->snd_buf != NULL && state_snd->snd_buf[state_snd->snd_no] != NULL)
...
void mixerFadeEffectInit (void) {
	...
	lin_fade = (FADE_REC *)calloc (sizeof(FADE_REC), no_sbuffs);
...
short mixerFadeEffect (short data, unsigned int pos, unsigned int j)
{
	...
	STATE_SND *state_snd = mixerGetStateSndPtr (j, sbuffs[j].vol);
	int fade_pt = (int)(lin_fade[j].fade_time * (double)STEREO * (double)SAMPLE_RATE);
	int remainder;
	...
	if (remainder > fade_pt)
		...
		if (remainder == fade_pt) {
			 ...
			 */
			stereo = (state_snd->pos % 2) ? (1.0 - sbuffs[j].stereo_pos) : sbuffs[j].stereo_pos;
			new = (double)sbuffs[j].thresh[lin_fade[j].old_thresh].state_snd.snd_buf[lin_fade[j].snd][lin_fade[j].pos] *
